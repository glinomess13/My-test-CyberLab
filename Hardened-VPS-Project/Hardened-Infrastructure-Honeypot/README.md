...
* **telegram_incident_alerts.png:** Live feed showing a storm of over 7.5k automated notifications with recorded attacker IPs and dictionary password attempts.
...
* **honeypot_attacker_frustration.png:** Forensic Docker log evidence capturing an automated botnet failing commands and sending malicious string responses (`—-fuck_you—-`) inside the emulated SSH space.
* **kali_terminal_log_parsing.png:** Screenshot from the VirtualBox Kali Linux environment showcasing active log parsing via the terminal using advanced Regex (`grep -oE`).
